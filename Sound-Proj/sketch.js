...
function preload(){
 ...
 // specify width and height of each frame and the number of frames
  ...
  // Load the sounds
  squishSound = new Tone.Player('media/squishsound.mp3').toDestination();
   skitterSound = new Tone.Player('media/skitter.mp3').toDestination();
   missSound = new Tone.Player('media/missed.mp3').toDestination();
  bgMusic = new Tone.Player('media/background.mp3').toDestination();
  gameoverSound = new Tone.Player('media/gameover.mp3').toDestination();
  tickSound = new Tone.Player('media/ticking.mp3').toDestination();
...
function updateMusicAndSounds() {

 

  let totalBugSpeed = 0;
  for (let bug of bugs) {
    totalBugSpeed += abs(bug.speedX) + abs(bug.speedY); // Measure total bug movement
  }
  
  
  // Speed up or slow down the skitter sound based on bug speed
  skitterSound.playbackRate = 1 + (totalBugSpeed / numBugs) * 0.1; // Adjust based on number of bugs and their speed
...
class Bug {
  ...
  skitterSound.start(); // Play the skitter sound
